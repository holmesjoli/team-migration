...
df_long <- df %>%
  tidyr::pivot_longer(names_to = "sub_mode_id", values_to = "values", -c(iso3, country, region)) %>% 
  tidyr::separate(sub_mode_id,
                  into = c("sub_mode_id", "type"),
                  sep = "([(_)])") %>%
  mutate(mode_id = substr(sub_mode_id, 1, 3),
         acq = grepl("A", sub_mode_id),
         loss = grepl("L", sub_mode_id),
         bin = "bin" == type,
         cat = "cat" == type,
