...
cntry_code <- xwalk %>% 
  distinct(region, loc_code, iso3)

subregion_code <- xwalk %>% 
  distinct(subregion, subregion_code)

continent_code <- xwalk %>% 
  distinct(continent, continent_code)
```
...
South America flow of people to other subregions of the world

```{r}
sa_flow <- flow %>% 
  filter(dest_loc %in% subregion_code$subregion_code) %>% 
  filter(orig_loc %in% subregion_code$subregion_code) %>% 
  right_join(subregion_code %>% 
    filter(subregion == "South America"), 
    by = c("orig_loc" = "subregion_code")) %>% 
  rename(subregion_orig = subregion) %>% 
  right_join(subregion_code, by = c("dest_loc" = "subregion_code")) %>% 
  rename(subregion_dest = subregion) %>%
  filter(!is.na(value)) %>%
  mutate(points = round(value/1000))
```
