...
cntrys <- unique(df_long$iso3)

z <- lapply(cntrys, function(i) {

  pos <- grep(i, cntrys)

  lapply(cntrys[pos+1:length(cntrys)], function(j) {
    ...
    df1 <- df_long %>% 
          filter(acq) %>% 
          filter(bin) %>% 
          filter(iso3 %in% c(i, j)) %>% 
      select(iso3, mode_id, values) %>%
      pivot_wider(names_from = iso3, values_from = values)
    
    pct <- sum(df1[[2]] == df1[[3]])/length(eq)
...
z2 <- z %>%
  mutate(id = seq(1, n(), 1)) %>%
  pivot_longer(names_to = "name", values_to = "iso3", -c(id, pct)) %>% 
  select(-name) %>% 
  left_join(df %>% select(iso3, country, region)) %>% 
  pivot_wider(names_from = id, values_from = iso3)
```
